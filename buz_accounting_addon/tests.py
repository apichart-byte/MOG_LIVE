...
class TestBuzAccountReceipt(TransactionCase):
    ...
    def test_receipt_autopost_functionality(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param('buz_accounting_addon.auto_post_receipts', True)
    ...
    def test_single_currency_enforcement(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param(
            'buz_accounting_addon.enforce_single_currency_per_receipt', True
        )
...
class TestPaymentVoucherApproval(TransactionCase):
    def setUp(self):
        super().setUp()
        
        # Create approver users
        self.checker1 = self.env['res.users'].create({
            'name': 'Checker 1',
            'login': 'checker1',
            'email': 'checker1@example.com',
            'groups_id': [(6, 0, [self.env.ref('base.group_user').id])]
        })
        self.checker2 = self.env['res.users'].create({
            'name': 'Checker 2',
            'login': 'checker2',
        ...
        # Configure company with approvers
        self.env.company.write({
            'payment_voucher_checker1_id': self.checker1.id,
            'payment_voucher_checker2_id': self.checker2.id,
            'payment_voucher_approver_id': self.approver.id
        })
    ...
    def test_approval_workflow_activity(self):
        """Test the multi-step approval workflow and activity generation"""
        
        # 1. Create Voucher
        voucher = self.env['account.payment.voucher'].create({
            'partner_id': self.vendor.id,
            'date': fields.Date.today(),
            'destination_journal_id': self.env['account.journal'].search([('type', '=', 'bank')], limit=1).id
        })
        
        # 2. Advance to Confirm (Draft -> Prepared)
        voucher.action_confirm()
        
        # Verify Activity for Checker 1
        activity_c1 = self.env['mail.activity'].search([
            ('res_id', '=', voucher.id),
            ('res_model', '=', 'account.payment.voucher'),
            ('user_id', '=', self.checker1.id),
            ('summary', 'ilike', 'Check (1)')
        ])
        self.assertTrue(activity_c1, "Activity should be created for Checker 1")
        
        # 3. Advance to Check 1 (Prepared -> Checked 1)
        voucher.action_check1()
        
        # Verify Activity for Checker 1 Done
        self.assertFalse(activity_c1.exists(), "Activity for Checker 1 should be done")
        
        # Verify Activity for Checker 2
        activity_c2 = self.env['mail.activity'].search([
            ('res_id', '=', voucher.id),
            ('res_model', '=', 'account.payment.voucher'),
            ('user_id', '=', self.checker2.id),
            ('summary', 'ilike', 'Check (2)')
        ])
        self.assertTrue(activity_c2, "Activity should be created for Checker 2")

        # 4. Advance to Check 2 (Checked 1 -> Checked 2)
        voucher.action_check2()

        # Verify Activity for Checker 2 Done
        self.assertFalse(activity_c2.exists(), "Activity for Checker 2 should be done")
        
        # Verify Activity for Approver
        activity_app = self.env['mail.activity'].search([
            ('res_id', '=', voucher.id),
            ('res_model', '=', 'account.payment.voucher'),
            ('user_id', '=', self.approver.id),
            ('summary', 'ilike', 'Approval Required')
        ])
        self.assertTrue(activity_app, "Activity should be created for Approver")
        
        # 5. Approve
        voucher.action_approve() # Checked2 -> Approved
        
        # 6. Verify Activity Done
        self.assertFalse(activity_app.exists(), "Activity for Approver should be done")