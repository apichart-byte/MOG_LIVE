...
class AgedPayableXlsx(models.AbstractModel):
    _name = 'report.buz_gl_report_excel.report_aged_payable_xlsx'
    _inherit = 'report.report_xlsx.abstract'

    def generate_xlsx_report(self, workbook, data, wizards):
        pdf_report = self.env['report.buz_gl_report_excel.report_aged_payable']
        ...
        sheet.merge_range('A1:L1', f"{company.name}: Aged Payable", title_fmt)
        sheet.write('A2', f"As of Date: {date_as_of}")
        if 'aged_payable_xlsx.py' == 'aged_partner_xlsx.py':
